...
namespace Landis.Extension.Succession.Landispro
{
    public class In_Output
    {
        ...
        //private static readonly uint PlugIn.gl_sites.SpecNum = PlugIn.gl_sites.SpecNum;

        //private static readonly int time_step = PlugIn.gl_sites.TimeStep;
        //private static readonly int pro0or401 = PlugIn.gl_sites.Pro0or401;

        //private static readonly string output_dir = PlugIn.gl_param.OutputDir;
        ...
        public static void SetAgeDistoutputBuffer_Landtype(int type, int specIndex, int ageIndex, int yearIndex, int landtypeIndex, double value)
        {
            ...
            long index = landtypeIndex * PlugIn.gl_sites.SpecNum * doubl_dim + (specIndex - 1) * doubl_dim + (ageIndex - 1) * dimension + yearIndex - 1;
        ...
        public static void initiateOutput_landis70Pro()
        {
            ...
            if (reMethods[0] != "N/A")
            {
                for (int i = 1; i <= PlugIn.gl_sites.SpecNum; ++i)
                {
            ...
            AgeDistOutputBufferInitialize(PlugIn.gl_sites.SpecNum, PlugIn.gl_landUnits.Num_Landunits);
        }
        ...
        public static void AgeDistOutputFromBufferToFile()
        {
            ...
            for (int k = 1; k <= PlugIn.gl_sites.SpecNum; ++k)
            {
        ...
        public static void putOutput_Landis70Pro(int rep, int itr, int[] freq)
        {
                    ...
                    if (IVFileNames[k - 1] != "N/A")
                    {
                        ...
                        for (uint i = (uint)row_num; i > 0; --i)
                        {
                            for (uint j = 1; j <= col_num; ++j)
                            {
                                ...
                                for (int kk = 1; kk <= PlugIn.gl_sites.SpecNum; ++kk)
                                {
            ...
            int Bio_flag = PlugIn.gl_sites.GetOutputGeneralFlagArray(PlugIn.gl_sites.SpecNum, (int)enum1.Bio);
            int Car_flag = PlugIn.gl_sites.GetOutputGeneralFlagArray(PlugIn.gl_sites.SpecNum, (int)enum1.Car);
            int  BA_flag = PlugIn.gl_sites.GetOutputGeneralFlagArray(PlugIn.gl_sites.SpecNum, (int)enum1.BA);
            int RDenflag = PlugIn.gl_sites.GetOutputGeneralFlagArray(PlugIn.gl_sites.SpecNum, (int)enum1.RDensity);
            int TPA_flag = PlugIn.gl_sites.GetOutputGeneralFlagArray(PlugIn.gl_sites.SpecNum, (int)enum1.TPA);
            ...
            if (PlugIn.gl_sites.FlagAgeRangeOutput != 0)
            {
                ...
                int Bio_endagerange_flag = PlugIn.gl_sites.GetOutputAgerangeFlagArray(PlugIn.gl_sites.SpecNum, (int)enum1.Bio);
                int Car_endagerange_flag = PlugIn.gl_sites.GetOutputAgerangeFlagArray(PlugIn.gl_sites.SpecNum, (int)enum1.Car);
                int  BA_endagerange_flag = PlugIn.gl_sites.GetOutputAgerangeFlagArray(PlugIn.gl_sites.SpecNum, (int)enum1.BA);
                int TPA_endagerange_flag = PlugIn.gl_sites.GetOutputAgerangeFlagArray(PlugIn.gl_sites.SpecNum, (int)enum1.TPA);
                int RDenendagerange_flag = PlugIn.gl_sites.GetOutputAgerangeFlagArray(PlugIn.gl_sites.SpecNum, (int)enum1.RDensity);
